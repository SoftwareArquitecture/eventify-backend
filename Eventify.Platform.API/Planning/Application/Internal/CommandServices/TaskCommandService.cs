...
public class TaskCommandService : ITaskCommandService
{
    ...
    public TaskCommandService(ITaskRepository taskRepository, IUnitOfWork unitOfWork, ILogger<TaskCommandService> logger)
    {
    ...
    public async Task<TaskItem?> Handle(UpdateTaskCommand command)
    {
        try
        {
            ...
            _logger.LogInformation("Current task: Title='{CurrentTitle}', Description='{CurrentDescription}', ColumnId={CurrentColumnId}, Order={CurrentOrder}", 
            ...
            _logger.LogInformation("Updated task: Title='{NewTitle}', Description='{NewDescription}', ColumnId={NewColumnId}, Order={NewOrder}", 
        ...
        {
            _logger.LogError(ex, "=== TaskCommandService.Handle(UpdateTaskCommand) - Error: {Message} - StackTrace: {StackTrace} ===", ex.Message, ex.StackTrace);
            throw;
        }
    }
