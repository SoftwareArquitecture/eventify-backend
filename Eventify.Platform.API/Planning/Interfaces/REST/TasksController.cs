...
public class TasksController : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateTask(int id, [FromBody] UpdateTaskResource resource)
    {
        try
        {
            ...
            var task = await _taskCommandService.Handle(command);
            _logger.LogInformation("Command service returned: {TaskResult}", task != null ? "Task found" : "Task not found");
        ...
        {
            _logger.LogError(ex, "=== UpdateTask - Error: {Message} - StackTrace: {StackTrace} ===", ex.Message, ex.StackTrace);
            return StatusCode(500, $"Internal server error: {ex.Message}");
        }
    }
